...
export function ProductActions({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-y-3 gap-x-4">
      {/* Price — discounted (big, accent) + original struck + discount % badge.
          Amounts are already rounded to whole taka (no decimals) server-side. */}
      <div className="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-0.5">
        ...
        {product.discount_price && (
          <>
            ...
            {discountPct > 0 && (
              <span className="text-sm font-semibold text-muted">-{discountPct}%</span>
            )}
          </>
        )}
      </div>
